...
![An image of a sphere where the dotted red line represents the great-circle distance between two points on the sphere’s surface. Notice that the great-circle circle distance is larger than a [direct straight line (Euclidean distance)](https://en.wikipedia.org/wiki/Euclidean_distance) through the sphere.](docs/img/The-great-circle-distance-between-origin-and-destination.png){ width=30% }
...
![This image depicts feature maps of the CNN trained on car images. We can see how the model learns progressively more complex features as we move higher in the layer order.](docs/img/example_feature_map_car.png){width=50%}
...
![Distribution of the dataset. It's visible that the mountainous parts of Croatia contains the least amount of locations in the dataset.](docs/img/croatia_data.png){width=50%}
...
![If you look closely at the red mesh, there are many small red pixels. These red pixels represent newly sampled locations. In this picture, there are 1 000 000 locations, so they’re overlapping](docs/img/sampling.png){width=50%}
...
![A map of Croatia. Although [we have a lot of islands](https://www.google.com/maps/@44.2652721,16.2377694,8.25z), the dataset we were provided with doesn’t include any Street View images taken on islands.](docs/img/croatia.png){width=50%}
...
![](docs/img/croata_n_20_grid.png){width=33%}
![](docs/img/croata_n_55_grid.png){width=33%}
![](docs/img/croata_n_115_grid.png){width=33%}
\begin{figure}[!h]
...
![\ ](docs/img/croatia_progression/country_HR_1.png){width=20%}
![\ ](docs/img/croatia_progression/country_HR_spacing_0.28_grid_2.png){width=20%}
![\ ](docs/img/croatia_progression/country_HR_spacing_0.28_grid_intersection_3.png){width=20%}
![\ ](docs/img/croatia_progression/data_fig__spacing_0.28__num_class_134_regions_colored_4.png){width=20%}
![\ ](docs/img/croatia_progression/data_fig__spacing_0.28__num_class_134centroid_5.png){width=20%}
\begin{figure}[!h]
...
![](docs/img/clipping_example.png){width=50%}
![](docs/img/no_images_in_class.png){width=50%}
\begin{figure}[!h]
...
![A random sample of images from the ImageNet dataset used for creating pretrained models.](docs/img/imagenet.jpg){width=30%}
...
![An example of the filters of the first convolutional layer learned by the pretrained ResNeXt network. We can recognize simple textures and contours.](docs/img/resnext_filters.png){width=50%}
...
![](docs/img/model_arh_high.png){width=50%}
![](docs/img/model_arh.png){width=50%}
\begin{figure}[!h]
...
![A graph of train model accuracy over time. A curve that is more to the left and up on the image shows faster training and higher accuracy.](docs/img/train_acc_epoch.png){width=100%}

![A graph of train model loss over time. A curve that is more to the left and down on the image shows quicker training and lower loss.](docs/img/train_loss_epoch.png){width=100%}
...
![A graph of validation model accuracy over time. A curve that is more to the left and up on the image shows faster training and higher accuracy.](docs/img/val_acc_epoch.png){width=100%}

![A graph of validation model loss over time. A curve that is more to the left and down on the image shows quicker training and lower loss.](docs/img/val_loss_epoch.png){width=100%}
...
![A graph of validation model haversine distance over time. A curve that is more to the left and down on the image shows quicker training and lower loss.](docs/img/val_hav_epoch.png){width=100%}

![A graph of validation model accuracy over time for our best model.](docs/img/val_acc_epoch_best.png){width=100%}
...
![A graph of validation model loss over time for our best model.](docs/img/val_loss_epoch_best.png){width=100%}
  
![A graph of validation model haversine distance over time for our best model.](docs/img/val_hav_epoch_best.png){width=100%}
